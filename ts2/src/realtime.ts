...
import { ChatGPTAuth, chatgptBaseUrl } from "./auth.ts";
...
export async function negotiate(
  ...
  if (response.status !== 200 && response.status !== 201) {
    throw new NegotiationError(`${response.status} from ${url}: ${text.slice(0, 500)}`);
  }
  return text;
}
...
export class RealtimeTranscriber {
  ...
  async connect(track: MediaStreamTrack): Promise<void> {
    ...
    channel.onMessage.subscribe((raw) => {
      let event: unknown;
      try {
        event = JSON.parse(typeof raw === "string" ? raw : raw.toString("utf-8"));
      } catch {
        return;
      }
    ...
    if (!offerSdp) {
      throw new NegotiationError("local description was not set after setLocalDescription");
    }
