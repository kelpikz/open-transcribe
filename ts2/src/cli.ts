...
import { listDevices, recordMicrophone } from "./audio.ts";
import {
  DEFAULT_LANGUAGE,
  DEFAULT_MODEL,
  TRANSCRIBE_MODELS,
...
export class Renderer {
  ...
  status(message: string): void {
    if (!this.enabled) return;
    process.stderr.write(`${this.clearLive()}${this.c(DIM)}  ${message}${this.c(RESET)}\n`);
  }
...
function renderFinal(text: string, args: Args): void {
  ...
  const renderer = new Renderer(true, false, Boolean(process.stderr.isTTY) && !args.noColor);
  renderer.final(text);
  renderer.done();
}
...
function applyValue(args: Args, name: string, value: string): void {
  switch (name) {
    ...
    case "--noise-reduction": {
      ...
      if (choice === undefined) {
        const options = NOISE_REDUCTION_CHOICES.map((option) => `'${option}'`).join(", ");
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  ...
  if (args.stream) {
    err("--stream is unavailable: the Codex Desktop upload route returns one final transcript.");
    return 2;
  }
  ...
  process.stdout.write(args.json ? `${JSON.stringify({ text })}\n` : `${text}\n`);
  return 0;
}
